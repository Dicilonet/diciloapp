...
const ClientProfile: React.FC<{ company: Company }> = ({ company }) => {
  ...
  const tabs = [
    { id: 'tours', label: t('client_profile.tabs.tours'), available: !!company.tours?.length },
      ...
      <main className="container mx-auto px-6 py-12">
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-8">
            {company.details?.strengthsKeys?.length > 0 && (
              <div className="bg-white p-6 rounded-lg shadow-md">
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const normalize = (str: string) => str.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/gi, '');
    const normalizedParam = companyName ? normalize(companyName) : '';
    const companyData = TOP_COMPANIES_DATA.find(c => normalize(c.name) === normalizedParam);
